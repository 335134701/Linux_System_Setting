...
function Remastersys_Install()
{
	...
	sudo cp ${HOME}/桌面/remastersys_2.0.11-1_all.deb remastersys_2.0.11-1_all.deb
:<<!
	count=$(ls *.deb | wc -w)
	if [ "${count}" -gt 0 ];then
		cd /home/${USER}
	...
	#更新软件
	#Update_All
	wget http://downloads.sourceforge.net/project/remastersys/remastersys-ubuntu/2.0.11/remastersys_2.0.11-1_all.deb
	Judge_Order "remastersys_2.0.11-1_all.deb download success" "remastersys_2.0.11-1_all.deb download fail" 0
!
	#remastersys_2.0.11-1_all.deb可由下载链接看出
	count=$(ls *.deb | wc -w)
	if [ ${count} -eq 1 ];then
		Update_All
		sudo dpkg -i remastersys_2.0.11-1_all.deb
		Judge_Order "remastersys_2.0.11-1_all.deb install success" "remastersys_2.0.11-1_all.deb install fail" 0
		sudo rm -rf remastersys_2.0.11-1_all.deb
	else 
		echo "The file remastersys_2.0.11-1_all.deb is not existence!"
		exit 127	
	fi
