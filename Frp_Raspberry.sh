...
function Judge_Order(){
	...
	if [ ${#} -eq 2 ];then
		...
		if [ ${status} -eq 0 ];then
			echo -e "[\033[32m$(date +"%Y-%m-%d %T") Info\033[0m]  ""\033[33m\"${1}\" \033[0m""\033[34mexecute success!\033[0m"
		else 
			echo -e  "[\033[32m$(date +"%Y-%m-%d %T") Info\033[0m]  ""\033[33m\"${1}\" \033[0m""\033[34mexecute fail!\033[0m"
			if [ "${2}" -eq 0 ];then
				exit 127
			fi
		fi	
...
{
	if [ ! -f ${filename} ];then
		echo -e "[\033[31m$(date +"%Y-%m-%d %T") Error\033[0m]  ""\033[31mThe file \"${filename}\" does not exist, the program is about to terminate execution!\033[0m"	
		exit 90
	fi
}
#配置文件修改
...
function Judge_Txt()
{
	...
	if [ ${#} -ge 1 ] && [ ${#} -le 3 ];then
		...
		echo -e "[\033[31m$(date +"%Y-%m-%d %T") Error\033[0m]  ""\033[31mIncorrect function parameter input, the program is about to terminate execution!\033[0m"
		return 80
	fi
	echo 
}
...
{
    echo
	test ! -f $(pwd)/frp_*.tar.gz && \
	echo -e "[\033[31m$(date +"%Y-%m-%d %T") Error\033[0m]  ""\033[31m文件$(pwd)/frp_*.tar.gz不存在!\033[0m" && \
	exit 90
	...
	Judge_Txt "\[vnc002\]"
	Judge_Txt "local_port = ${vnc_local_port}"
	Judge_Txt "remote_port = ${vnc_remote_port}"
    echo 
}
...
function Main()
{
	...
	#ssh对应的端口号，根据实际修改
	ssh_remote_port=6500
	#ssh本地对应端口
	ssh_local_port=22
	#VNC远程对应端口
	vnc_remote_port=5902
	#VNC本地对应端口
	vnc_local_port=5900
	#*********************************************************
	#第一步:执行欢迎方法
	...
	#第二步:安装Client端
	#echo
	#echo -e "[\033[33m$(date +"%Y-%m-%d %T") Warning\033[0m]  ""If the input options below are incorrect, they will be executed according to the default selection!"
	echo
	...
	echo -e "[\033[32m$(date +"%Y-%m-%d %T") Info\033[0m]  ""启动frp方法:执行 ${ClientDir}/frp.sh  !"
